...
namespace IBM.Watsson.Examples
{
    public class ExampleStreaming : MonoBehaviour
    {
        ...
        void Awake()
        {
            lightCommand = light.GetComponent<Light>();
                    ...
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        foreach(var word in alt.transcript.Split(' '))
                        {
                            if (commands.Contains(word.ToLower()))
                            {
                                ...
                                if(word.ToLower().Contains("play"))
                                {
                                    animation.Play("Figure");
                                    Debug.Log("RUN");
                                }

                                if(word.ToLower().Contains("stop"))
                                {
                                    animation.Play("Stop");
                                    Debug.Log("STOP");
                                }

                                if(word.ToLower().Contains("light"))
                                {
                                    lightCommand.intensity = 1f;
                                }

                                if(word.ToLower().Contains("dark"))
                                {
                                    lightCommand.type = LightType.Spot;
                                    lightCommand.intensity = 0f;
                                }
                                // Spanish - es-MX_BroadbandModel
